create or replace function labor_empleado() returns trigger as $$
declare
    temporal record;
    dia record;
...
begin
    ...
    for temporal in select l.id,(EXTRACT(EPOCH FROM l.fin)-EXTRACT(EPOCH FROM l.ini))/3600.0 as tiempo,
                           ...
                           from operacion_labor as l
                            inner join usuario_empleado as e on(l.id=old.id and l.empleado_id=e.usuario_ptr_id)
    ...
    loop
        ...
        loop
		      ...
		      inner join empresa_tienda as t on (t.id=e.tienda_id and e.usuario_ptr_id=temporal.empleado)
		      inner join empresa_empresa as em on (em.user_ptr_id=t.empresa_id)
                      inner join operacion_configuracion as c on (em.ciudad_id=c.ciudad_id and t.empresa_id=c.empresa_id)
