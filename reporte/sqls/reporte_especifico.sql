...
begin
    ...
    select t1.orden, (
                      select min(bus.ini) from operacion_labor as bus where (extract(day from bus.ini)=t1.orden and cast(bus.ini as date) >= inicio and cast(bus.ini as date) <= finalizar) and  bus.empleado_id=t1.empleado) as ini,
                      (select max(bus.fin) from operacion_labor as bus where (extract(day from bus.ini)=t1.orden and cast(bus.fin as date) >= inicio and cast(bus.fin as date) <= finalizar ) and bus.estado=true and  bus.empleado_id=t1.empleado) as fin,sum(case when t1.total is not null then t1.total else 0 end) as total_horas,sum(case when t1.horas is not null then cast(t1.horas as numeric) else 0 end) as horas,t1.empleado
