-- Function: get_total_trabajador2(integer, text, integer, integer, integer, date, date)

-- DROP FUNCTION get_total_trabajador2(integer, text, integer, integer, integer, date, date);

CREATE OR REPLACE FUNCTION get_total_trabajador2(
...
begin
	return (SELECT COALESCE(array_to_json(array_agg(row_to_json(p))), '[]') from (
		select
		 ...
		 inner join motorizado_infomoto as im on (u.id=im.empleado_id and im.empleado_id and im.estado=true)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION get_total_trabajador2(integer, text, integer, integer, integer, date, date)
  OWNER TO postgres;
