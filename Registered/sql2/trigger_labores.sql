create or replace function trigger_labor() returns trigger as $$
declare 
  temporal record;
    dia record;
...
begin
  raise notice 'eso es lo q hay';
    delete from operacion_pagolabor where labor_id = new.id;
    ...
    for temporal in select l.id,(EXTRACT(EPOCH FROM l.fin)-EXTRACT(EPOCH FROM l.ini))/3600.0 as tiempo,
                 l.ini - interval '5 hours' as ini,l.fin - interval '5 hours' as fin,t.empresa_id as empresa,
                           ...
                           from operacion_labor as l 
                            inner join usuario_empleado as e on(l.empleado_id=e.usuario_ptr_id and l.id=new.id)
